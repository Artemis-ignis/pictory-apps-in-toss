...
- [ ] `npm run test`, `npm run typecheck`, `npm run lint`, `npm run qa:server`, `npm run qa:server:built`, `npm run check:production-env -- --file .env.production`, `npm run check:device-evidence -- --file qa-evidence/device-smoke.json`, `npm run build`, `npm run check:release`가 통과했다.
...
서버 AI 요청은 한 번에 최대 40장으로 제한한다. 영수증, 문서, 쿠폰, 캡처, 인물, 민감정보 후보는 원본/썸네일 이미지를 서버로 붙이지 않고 redacted 신호와 힌트만 보낸다. 이미지가 꼭 필요한 낮은 민감도 후보(음식, 장소, 기록)만 512px JPEG로 줄여 전송한다. 서버의 기본 OpenAI 이미지 detail은 `low`로 두어 비용을 방어한다.
...
- 앨범 권한 요청
- 이미지 리사이즈, redacted 전송 결정, 1차 분류
- 브라우저 네이티브 감지기로 QR/바코드, 얼굴, 텍스트 영역 힌트 추출
- 사용자 크레딧/플랜 상태 표시
- 서버가 준 분류 결과 표시
- 원본 사진 저장 금지와 기존 저장 raw preview 정화
...
서버 AI 정밀 분류 quota는 `server/pictoryUsageLedger.ts`의 서버 원장 기준으로 계산한다. 프론트엔드 localStorage의 `credits`와 `planId`는 UI 표시와 로컬 1차 정리 한도용이며, 서버 AI 비용이 발생하는 요청에서는 권위 있는 값으로 보지 않는다.
