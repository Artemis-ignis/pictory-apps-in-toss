...
- 원본 사진은 저장하지 않습니다.
- 최근 분류 결과는 앱 안의 로컬 저장소에 작은 썸네일 또는 차단용 대체 이미지와 분류 메타데이터만 보관합니다.
- 민감정보, 확인 필요, 영수증, 문서, 쿠폰 후보는 실제 이미지 썸네일 대신 차단용 대체 이미지를 저장합니다.
- 기존 버전에 저장된 큰 preview `dataUri`는 로드 시 비워 원본 사진이 남지 않게 정화합니다.
